# The main part of KGAT from Dr.He's Github. I only complete the model and make sure the KGAT model can run. After upload
# the pretraining dataset, I got the outcomes for my disseration.
...
class KGAT(object):
    def __init__(self, data_config, pretrain_data, args):
        ...
        # Create PlaceHolder parts for store Input Data and Dropout data
        self._build_inputs()

        # Create CF's parameters
        ...
        *********************************************************
        ...
        # optimize CF's loss function by BRP Loss

        self._build_loss_phase_I()

        # TransR to computer KG
        self._build_model_phase_II()

        # optimize the kG's part
    ...
    def _build_inputs(self):
        ...
        # On embedding knowledge graph with TransD method
        ...
        # outcomes: node information on the ego-networks
    ...
    def _get_kg_inference(self, h, r, pos_t, neg_t):
        ...
        # head entity and tail entity embeddings part with batch_szie * embedding_dim *21
        ...
        # relation embedding match: batch_size * KGE dimension
        r_e = tf.nn.embedding_lookup(self.weights['relation_embed'], r)

        # relation transform weights: batch_size * KGE dimension * embedding dimension
        trans_M = tf.nn.embedding_lookup(self.weights['trans_W'], r)

        # batch_size * 1 * KGE dimension
    ...
    def _build_loss_phase_II(self):
        ...
        # Using the softmax as BRP loss function
    ...
    def _create_bi_interaction_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            # Sum the neighbors information
            side_embeddings = tf.concat(temp_embed, 0)

            add_embeddings = ego_embeddings + side_embeddings

            # transformed all neighbors information
            sum_embeddings = tf.nn.leaky_relu(
                tf.matmul(add_embeddings, self.weights['W_gc_%d' % k]) + self.weights['b_gc_%d' % k])


            # bi messages of neighbors.
            # bi-interaction part, to calculate the neighbors
            ...
            # normalize the neighbours distribution of embedding
    ...
    def _create_graphsage_embed(self):
        ...
        for k in range(self.n_layers):
            ...
            # this is for RM_GCN to get the weighted sum
            temp_embed = []
            for f in range(self.n_fold):
                temp_embed.append(tf.sparse_tensor_dense_matmul(A_fold_hat[f], pre_embeddings))
            embeddings = tf.concat(temp_embed, 0)

            # line 2 for RM_GCN, aggregating the former embedding parts
            ...
            # do normalization for distribute of embedding
            norm_embeddings = tf.math.l2_normalize(embeddings, axis=1)
    ...
    def _generate_transE_score(self, h, t, r):
        ...
        # get the relation embeddings: batch_size * KGE dimension
        r_e = tf.nn.embedding_lookup(self.weights['relation_embed'], r)

        # get the relation transform weights: batch_size with KGE dimension with embedding dimension
        trans_M = tf.nn.embedding_lookup(self.weights['trans_W'], r)

        # batch_size times KGE dimension equals batch_size times KGE dimension
        h_e = tf.reshape(tf.matmul(h_e, trans_M), [-1, self.kge_dim])
    ...
    def _statistics_params(self):
        ...
        for variable in self.weights.values():
            # the shape of an array of tr.Diemsion
    ...
    # The part to upload the attentive laplacian matrix details
